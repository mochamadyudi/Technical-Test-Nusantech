...
const Page: React.FC = () => {
  ...
  const languages = useSelector<any, ITmdbState['languages']>(
    (state) => state.tmdb.languages
  )
  const state = useSelector<any, ITmdbState['movies']['show']>(
    (state) => state.tmdb.movies.show
  ) as ITmdbState['movies']['show']
  ...
  return (
    <React.Fragment>
      <div className="w-full app-content-full app-page-pt flex flex-col overflow-hidden relative">
        {state?.loading ? (
          <ContentLoading data-testid="loading-spinner" />
        ) : state.data !== null ? (
          <React.Fragment>
            <section className="flex flex-col items-center pt-10 flex-1">
              <TheContainer className="z-10 pt-20 space-y-20 flex flex-col flex-1">
                <div className="space-y-10 flex-1">
                  <Typography.Title level={1} className="text-7xl">
                      ...
                      !isNaN(state.data?.vote_average) && (
                        <React.Fragment>
                          <Rate
                            disabled
                            defaultValue={
                              Math.round((state.data.vote_average / 2) * 2) / 2
                            }
                            value={
                              Math.round((state.data.vote_average / 2) * 2) / 2
                            }
                    ...
                    {state.data?.popularity &&
                      !isNaN(state.data?.popularity) && (
                        <span className="flex items-center gap-1">
                          ...
                          />
                          <Typography>
                            {formatNumberShort(state.data.popularity)}
                          </Typography>
                        </span>
                      )}
                  </Flex>
                  {
                    state.data?.status === 'Released' && (
                      <Typography>
                        Release {dayjs(state?.data?.release_date, 'YYYY-MM-DD').isValid() ? dayjs(state?.data?.release_date, 'YYYY-MM-DD').format('DD MMMM, YYYY'): '-'}
                      </Typography>
                    )
                  }
          ...
          <div className="flex-1 flex items-center justify-center">
            <Result status="404" title="Movie not found" />
          </div>
        )}
      </div>
      <CastingSection />
