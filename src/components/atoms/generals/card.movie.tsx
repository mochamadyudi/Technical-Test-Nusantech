      ...
      >
        <Link to={`/movies/${data.id}`}>
          <div className="overflow-hidden rounded-xl">
            <img
              src={HttpUtil.getAssetTmdb(data?.poster_path, {
                width: 150,
                height: 150,
                platform: 'bestv2'
              }) ?? ''}
          ...
          type === "card-post" && (
            <div className={Classes(
              ...
              <div className="h-10 pt-2">
                <Typography className="truncate text-lg text-white !font-bold !font-pro-bold">{data.title}</Typography>
              </div>
              <div className="flex items-center justify-between">
                <Typography>{dayjs(data.release_date, 'YYYY-MM-DD').year()}</Typography>
                <div className="flex items-center gap-2">
                  {
                    data?.popularity &&
                    !isNaN(data?.popularity) && (
                      <span className="flex items-center gap-1">
                      <Icons type={'HeartFilled'} className="text-rose-500"/>
                      <Typography>{formatNumberShort(data.popularity)}</Typography>
                    </span>
                    )
                  }
                  {
